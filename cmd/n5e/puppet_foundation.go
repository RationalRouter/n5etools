...
// fresh every render" treatment as puppetIntegratedWeaponAttack, but reading
// expectedScores (loadPuppetsTabData's own fully-folded ExpectedStr/Dex/
// Con/Int/Wis/Cha — class baseline, Foundation pick, Elevated Design,
// Puppet Tool ASI, and Symphony of Puppetry's Enhancement branch, all
// combined) rather than the character's Ninjutsu/Taijutsu modifier, per
// each entry's own "+ ability Modifier [+ your Proficiency Bonus]" text.
// Deliberately not the companion's own stored score: every one of those
// bonuses applies the moment it's picked, with nothing left to separately
// sync into a stored field — this is the same reason a raw
// companionAbilityModifier read against the stored field used to
// under-report the modifier. nil for a pick with no natural weapon at all
// (Bestial/Matryoshka Framework, every Role, Sentinel).
